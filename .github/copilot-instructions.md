...
- **私有方法支援**：可透過勾選框選擇是否顯示和呼叫 private 方法，方便測試和除錯
...
- **穩定性保護**：自動偵測並跳過不支援的複雜泛型類型（Dictionary、HashSet、Queue、Stack），防止編輯器崩潰
...
- 序列化與反序列化的正確性
- 邊界情況（null 值、空陣列等）
- 包含複雜泛型類型（Dictionary、HashSet等）的組件的穩定性
- 遞迴建構子的安全處理
...
### 穩定性與錯誤處理
- **反射異常保護**：所有反射操作都包含 try-catch 保護，防止未預期的崩潰
- **類型安全檢查**：自動跳過不支援的複雜泛型類型（Dictionary、HashSet、Queue、Stack）
- **遞迴建構子保護**：檢測並安全處理只有遞迴建構子的類型（如建構子參數為自身類型），避免堆疊溢位
- **優雅降級**：遇到問題時會記錄警告並繼續運行，而非中斷編輯器


## 開發步驟
當你開發一個新功能時，請以最適合測試的方式進行開發，不管是寫程式碼還是其他流程。完成後，請撰寫足夠的測試，最後必須在 `copilot-instructions.md` 中補充功能，讓其他開發者能夠知道目前有哪些功能。